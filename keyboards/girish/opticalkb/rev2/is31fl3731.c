...
void is31fl3731_init(void) {
    ...
    if (!initialized) {
        initialized = true;
        palSetLineMode(RP_I2C0_SDA_PIN, PAL_MODE_ALTERNATE_I2C |
                                            PAL_RP_PAD_PUE | PAL_RP_PAD_DRIVE4);
        palSetLineMode(RP_I2C0_SCL_PIN, PAL_MODE_ALTERNATE_I2C |
                                            PAL_RP_PAD_PUE | PAL_RP_PAD_DRIVE4);
...
void is31fl3731_all_led_on(uint8_t brightness_level) {
    ...
    //     i2c_reg_write_uint8(i, 0xff); // turn on all LED
    // }
